...
@click.option(
    ...
    type=click.Path(exists=True),
    required=True,
    help="Path to keyword dictionary file (CSV or JSON)",
)
@click.option(
    "--domain",
    "-d",
    ...
    default="dissatisfaction",
    help="Text domain for the imported conditions",
)
@click.option(
    "--output",
    "-o",
    type=click.Path(),
    default=None,
    help="Output path for the imported condition set (YAML). "
    "Defaults to <input_stem>_imported.yaml",
)
def import_keywords(input: str, domain: str, output: str | None) -> None:
    """Import a keyword dictionary from CSV or JSON into a ConditionSet.

    CSV format: condition,keyword,weight,notes

    JSON format: {"domain": "...", "conditions": [...], "outcome": {...}}
    """
    from pathlib import Path

    from experiment_engine.text_calibration.condition import save_condition_set
    ...
    fmt = input_path.suffix.lower()

    if fmt == ".csv":
        cs = import_keywords_csv(str(input_path), domain=domain)
    elif fmt == ".json":
        cs = import_keywords_json(str(input_path))
    else:
        console.print(f"[red]✗[/] Unsupported format: {fmt}. Use .csv or .json")
        sys.exit(1)

    # Determine output path
    out_path = output or str(input_path.with_suffix("").with_suffix(".yaml")).replace(
        ".yaml", "_imported.yaml"
    )
    save_condition_set(cs, out_path)

    console.print(
        f"[green]✓[/] Imported {len(cs.conditions)} condition(s) from {input_path.name}"
    )
    for cond in cs.conditions:
        console.print(f"  {cond.name}: {len(cond.keywords)} keyword(s)")
    if cs.outcome:
        console.print(
            f"  [yellow]Outcome:[/] {cs.outcome.name} "
            f"({len(cs.outcome.keywords)} keyword(s))"
        )
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Output path for the keyword dictionary file. "
    "Defaults to <condition_set_stem>_keywords.csv",
)
@click.option(
    "--format",
    "-f",
...
def export_keywords(condition_set: str, output: str | None, fmt: str) -> None:
    """Export a ConditionSet's keyword dictionary to CSV or JSON.

    CSV output columns: condition, keyword, weight, notes

    JSON output structure mirrors the import format with conditions + outcome.
    """
    from pathlib import Path

    from experiment_engine.text_calibration.condition import load_condition_set
    from experiment_engine.text_calibration.keyword_io import (
        export_keywords_csv,
    ...
    if output:
        ...
        out_path = str(cs_path.with_suffix("")) + f"_keywords{ext}"

    if fmt == "csv":
        count = export_keywords_csv(cs, out_path)
        console.print(f"[green]✓[/] Exported {count} keyword row(s) to {out_path}")
    else:
        export_keywords_json(cs, out_path)
        total_kw = sum(len(c.keywords) for c in cs.conditions)
        if cs.outcome:
            total_kw += len(cs.outcome.keywords)
        console.print(
            f"[green]✓[/] Exported {len(cs.conditions)} condition(s), "
            f"{total_kw} keyword(s) to {out_path}"
        )

...
- **BERT 是关键词匹配的补充，不是替代**。关键词词典提供高可解释性（用户可以精确知道哪些词在影响分数），BERT 提供语义覆盖率（捕捉词典遗漏的同义表达和隐含语义）。
- 用户**不需要自己上传模型**——工具应预置或从 CDN 自动加载轻量级中文模型。
- 用户应能**按条件粒度**选择匹配方式：条件 A 用关键词（因为有清晰的术语词典），条件 B 用 BERT（因为语义复杂难以用关键词穷举）。
- 默认推荐：先用关键词匹配跑一次，再启用 BERT 验证差异——如果两者结果高度一致，说明词典质量过关；如果不一致，需审查词典覆盖率或 BERT 的"黑盒判断"。
...
- [ ] **P1-32: BERT 语义校准产品定位与交互设计** [用户故事] 作为研究者，我需要明确理解三者的区别和选用场景：(1) 关键词匹配——透明、可解释、维护词典；(2) BERT 语义匹配——自动、覆盖隐含语义但黑盒；(3) 混合模式——加权平均，可调权重。产出：Settings 页面条件级 scoring_source 下拉框 + BERT 使用指南。(工作量: M, 来源: 客户代表)

- [ ] **P1-33: BERT 中文语义模型 Pyodide 可行性 PoC** [用户故事] 作为用户，我不需要自己上传模型。工具应预置轻量级中文 BERT（如 paraphrase-multilingual-MiniLM-L12-v2，约 120MB），从 CDN 自动加载。需验证：WASM 加载时间、单条文本推理延迟（期望 <500ms）、内存占用、与 Pyodide/sentence-transformers 的兼容性。PoC 产出：可行/不可行判断 + 性能数据。(工作量: M, 来源: 客户代表)
...
### BERT 深度集成（客户代表分析新增，依赖 P1-33 PoC 通过）

- [ ] **P2-25: 条件级"混合模式"配置** [用户故事] 作为研究者，我对"不满"条件使用关键词匹配（有清晰的术语词典），对"信任"条件使用 BERT（语义复杂难穷举）。Settings 每个条件旁应有 scoring_source 下拉框：keyword | bert | hybrid，hybrid 模式下显示权重滑块。(工作量: L, 来源: 客户代表, 依赖: P1-33)

- [ ] **P2-26: BERT 语义匹配可解释性视图** [用户故事] 使用 BERT 时，Results 需展示每条文本的注意力高亮片段，帮助我理解模型为何给出高/低隶属度（黑盒内部可解释性补偿）。(工作量: XL, 来源: 客户代表, 依赖: P1-33)
...
3. P1-32 + P1-33 + P1-34 (客户代表新增：BERT 路线图 + 词典编辑器)

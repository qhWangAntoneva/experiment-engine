...
**背景**：`handle_calibrate` 中 `process_with_outcome()` 调用导致 outcome 列退化为 0/1，implementing agent 未意识到算法语义错误，reviewer agent 也未检出。三位 expert agent 从代码审查流程、测试策略、架构设计三个角度提出了预防方案。**审查流程规则（输出假设记录/多路径审查/质疑指令/语义测试原则）已写入 `~/.claude/CLAUDE.md` 和项目级 `CLAUDE.md`，此处仅保留需要实际代码修改的条目。**

- [ ] **P2-40: [Critical][M] 合并 process/process_with_outcome 为单方法 + OutcomeHandling 枚举** — 消除双方法 API 误用可能性：`process(data, outcome_handling=OutcomeHandling.CALIBRATE_FROM_TEXT)`，`outcome_vector` 参数仅在 `OVERRIDE_WITH_VECTOR` 模式时有效。涉及文件：`calibrator.py`, `pyodide_handlers.py`, `api.py`。（来自: 架构专家）
- [ ] **P2-42: [High][S] MembershipData 增加 fsQCA 列退化运行时检查** — `MembershipData` Pydantic validator 增加：对于 fsQCA variant，如果某列只有 {0.0, 1.0} 两个唯一值，抛出 ValueError。这会在校准完成后立即检测到退化。涉及文件：`models/qca.py`。（来自: 架构专家）
- [ ] **P2-45: [High][L] StageContract 管道契约验证** — 引入 `StageContract` mixin 基类，每个 pipeline stage 实现 `validate_input()` / `validate_output()`。TextCalibrationStage 输出契约检查：所有列在 fsQCA 模式下必须至少有 3 个唯一值。Pipeline 运行时自动调用。涉及文件：`pipeline.py`, `calibrator.py`。（来自: 架构专家）
- [ ] **P2-46: [Medium][S] validate_qca_output.py 增强 — outcome 列退化标记为 ERROR** — 当前仅检查唯一值 < 2 时警告，改为：outcome 列唯一值 <= 2 且子集为 {0, 1} 时标记为 ERROR（非 WARNING）。涉及文件：`validate_qca_output.py`。（来自: 测试策略专家）
...
| E. 流程改进（事件复盘） | 4 | P2-42/46 (S) → P2-40 (M) → P2-45 (L) |
| **合计** | **31** | **S: 7 / M: 13 / L: 8 / XL: 3** |

**推荐执行顺序**：先做 S 级快速修复（P2-28/31/33/37/26/42/46）→ M 级改进（P2-25/34/35/30/40/43）→ L 级集成（P2-29/36/32/45）→ XL 架构（P2-27）。

...
- [2026-05-24/26] **pre-commit stash-conflict infinite loop (UPDATED 2026-05-26)**: The original fix (`.wolf/hooks/pre-commit.js`) only ran `ruff format . && ruff check --fix . && git add -u` -- it did NOT cover `end-of-file-fixer`, `trailing-whitespace`, or other non-ruff hooks. When non-Python files (e.g. memory.md, token-ledger.json) lacked trailing newlines or had trailing whitespace, ruff (Python-only) ignored them, `git add -u` staged broken files, and the real git pre-commit hook's `end-of-file-fixer` modified them post-stage. This created unstaged modifications that conflicted with pre-commit's stash/unstash mechanism, causing repeated failures. **Updated fix (2026-05-26)**: `.wolf/hooks/pre-commit.js` now runs `uv run pre-commit run --all-files` (covering ALL 8 hooks) before `git add -u`, ensuring every pre-commit fix is applied and staged before the actual git commit executes.

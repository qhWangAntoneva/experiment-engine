...
- [2026-05-24] **Pyodide worker handler 模式**：所有 Python 逻辑应放在 `src/experiment_engine/pyodide_handlers.py` 的独立函数中，通过 `FS.writeFile → runPythonAsync('import & call') → FS.readFile` 模式与 worker 通信。严禁在 worker 中嵌入 Python 代码字符串。worker 中的 `runHandler()` 模板函数封装了此模式。
- [2026-05-24] **pyodide.worker.ts 原始 handleCalibrate 存在 _fuzzy_data 累积 bug**：变量初始化为 None 后从未更新，导致只处理最后一个样本，且 json.dumps 会失败。正确做法是提取 membership/case_ids/texts 并分步重建 FuzzySetData。
- [2026-05-24] **qca_reporter.py 的类是 QCALaTeXReporter 而非 QCAReporter**：worker 中硬编码的 import 名字不匹配会导致 ImportError。
...
- [2026-05-25] **P0-9 统一校准管道（Python 侧）**：核心决策——不删除 PROTOTYPE 代码，采用渐进式废弃策略（deprecation comment + 保留逻辑）。`handle_calibrate` 新增可选第4参数 `prototype_texts_path=None`：3 参数调用行为完全不变（向后兼容），4 参数调用同时校准 raw + prototype 文本并返回两个 MembershipData。`handle_calibrate_prototype` 变为 wrapper 委托给统一 handler。`ScoringSource.PROTOTYPE` 标记为废弃但枚举值保留。calibrator.py 的 PROTOTYPE 分支保留逻辑仅添加废弃注释。关键理由：(1) 20 个 prototype 测试全部通过，无需修改；(2) 旧 TS 代码调用 3-arg 签名完全向后兼容；(3) 新 TS 代码（待前端合并）可通过 4-arg 签名同时获取两份数据。

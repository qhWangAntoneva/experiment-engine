...
class TestKeywordMatcher:
    """Tests for Chinese text cleaning, tokenization, and keyword matching."""

    def test_clean_text_removes_punctuation(self):
        """Chinese punctuation and whitespace should be removed."""
        text = "政策很好，但是落实不力！"
        result = KeywordMatcher.clean_text(text)
    ...
    def test_clean_text_preserves_chinese_chars(self):
        ...
        text = "公民反馈机制非常重要"
        result = KeywordMatcher.clean_text(text)
        # All Chinese chars preserved
        assert len(result) == len("公民反馈机制非常重要")

    def test_clean_text_empty_input(self):
        """Empty input returns empty string."""
        assert KeywordMatcher.clean_text("") == ""

    def test_clean_text_only_punctuation(self):
        """Text with only punctuation returns empty string."""
        assert KeywordMatcher.clean_text("，。！？、；：") == ""

    def test_tokenize_bigram(self):
        """Bigram tokenization produces correct 2-char tokens."""
        tokens = KeywordMatcher.tokenize("政策执行", method="bigram")
        assert len(tokens) == 3
        assert tokens == ["政策", "策执", "执行"]

    def test_tokenize_unigram(self):
        """Unigram produces single char tokens."""
        tokens = KeywordMatcher.tokenize("政策", method="unigram")
        assert len(tokens) == 2
        assert tokens == ["政", "策"]

    def test_tokenize_trigram(self):
        """Trigram produces 3-char tokens."""
        tokens = KeywordMatcher.tokenize("政策执行力", method="trigram")
        assert len(tokens) == 3
        assert tokens == ["政策执", "策执行", "执行力"]

    def test_tokenize_short_text(self):
        """Text shorter than n should return single token."""
        tokens = KeywordMatcher.tokenize("政", method="bigram")
        assert tokens == ["政"]

    def test_tokenize_default_is_bigram(self):
        """Default tokenization method is bigram."""
        tokens = KeywordMatcher.tokenize("政策", method="invalid_or_default")
        # Falls through to bigram default
        assert tokens == ["政策"]

    def test_score_single_with_known_keyword(self):
        """Text containing keyword should get positive score."""
        text = "公民参与反馈机制很重要"
        entries = [
            KeywordEntry(pattern="公民", weight=1.0, scope="exact"),
            KeywordEntry(pattern="反馈", weight=1.0, scope="exact"),
        ]
        score = KeywordMatcher.score_single(text, entries)
        assert score >= 2.0  # both keywords found

    def test_score_single_no_match(self):
        """Text without keywords scores 0."""
        text = "完全无关的内容"
        entries = [
            KeywordEntry(pattern="公民反馈", weight=1.0, scope="bigram"),
            KeywordEntry(pattern="政策", weight=1.0, scope="exact"),
        ]
        score = KeywordMatcher.score_single(text, entries)
        assert score == 0.0

    def test_score_single_weighted(self):
        """Weighted keywords contribute proportionally."""
        text = "政策"
        entries = [
            KeywordEntry(pattern="政策", weight=2.5, scope="exact"),
        ]
        score = KeywordMatcher.score_single(text, entries)
        assert score == 2.5

    def test_score_single_regex_scope(self):
        """Regex scope matches patterns."""
        text = "公民123参与456反馈"
        entries = [
            KeywordEntry(pattern=r"\d+", weight=1.0, scope="regex"),
        ]
        score = KeywordMatcher.score_single(text, entries)
        assert score == 2.0  # "123" and "456"

    def test_score_single_bad_regex_handled(self):
        """Bad regex pattern should not crash."""
        text = "随意文本"
        entries = [
            KeywordEntry(pattern="[bad", weight=1.0, scope="regex"),
        ]
        score = KeywordMatcher.score_single(text, entries)
        assert score == 0.0


class TestChineseKeywordDictionary:
    """Tests for the ChineseKeywordDictionary class."""

    def test_add_and_match_single(self):
        """Add a condition and match against a single text."""
        d = ChineseKeywordDictionary()
        d.add_condition(
            "satisfaction",
            [
                KeywordEntry(pattern="满意", weight=1.0, scope="exact"),
                KeywordEntry(pattern="好评", weight=1.0, scope="exact"),
            ],
        )
        result = d.match_text("这个服务质量很满意")
        assert result["satisfaction"] >= 1.0

    def test_match_corpus_shape(self):
        """match_corpus returns correct shape (n_texts, n_conditions)."""
        d = ChineseKeywordDictionary()
        d.add_condition("A", [KeywordEntry(pattern="政策", weight=1.0, scope="exact")])
        d.add_condition("B", [KeywordEntry(pattern="法律", weight=1.0, scope="exact")])

        texts = ["政策法规很重要", "法律规章制度", "完全不相关"]
        matrix = d.match_corpus(texts)
        assert matrix.shape == (3, 2)

    def test_match_corpus_values(self):
        """Match corpus correctly scores each text-condition pair."""
        d = ChineseKeywordDictionary()
        d.add_condition(
            "policy", [KeywordEntry(pattern="政策", weight=1.0, scope="exact")]
        )
        d.add_condition(
            "law", [KeywordEntry(pattern="法律", weight=1.0, scope="exact")]
        )

        texts = ["政策出台", "法律实施", "政策和法律"]
        matrix = d.match_corpus(texts)

        # text 0: "政策" found, "法律" not
        assert matrix[0, 0] >= 1.0
        assert matrix[0, 1] == 0.0
        # text 1: "法律" found, "政策" not
    ...
    def test_condition_names_property(self):
        """condition_names returns registered condition names."""
        d = ChineseKeywordDictionary()
        d.add_condition("A", [])
        d.add_condition("B", [])
        assert sorted(d.condition_names) == ["A", "B"]
    ...
    def test_remove_condition(self):
        ...
        d.add_condition("A", [])
        d.add_condition("B", [])
        d.remove_condition("A")
        assert d.condition_names == ["B"]

    def test_remove_nonexistent_condition(self):
        """Removing non-existent condition does not raise."""
        d = ChineseKeywordDictionary()
        d.remove_condition("Z")  # should not raise

    def test_match_text_empty_conditions(self):
        """Empty dictionary returns empty dict."""
        d = ChineseKeywordDictionary()
        result = d.match_text("任何文本")
        assert result == {}

    def test_match_corpus_empty_conditions(self):
        """Empty dictionary match_corpus returns zero-column matrix."""
        d = ChineseKeywordDictionary()
    ...
    def test_load_from_conditions(self):
        ...
        cond = ConditionDefinition(
            ...
            domain=TextDomain.DISSATISFACTION,
            keywords=[
                KeywordEntry(pattern="测试", weight=1.0, scope="exact"),
            ],
        )
        d = ChineseKeywordDictionary()
        d.load_from_conditions([cond])
        assert "test_cond" in d.condition_names

    def test_empty_match_corpus(self):
        """match_corpus on empty text list returns empty matrix."""
        d = ChineseKeywordDictionary()
        d.add_condition("A", [KeywordEntry(pattern="政策", weight=1.0, scope="exact")])
        matrix = d.match_corpus([])
